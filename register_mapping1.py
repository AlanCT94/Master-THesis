...
if __name__ == '__main__':
    ...
    # Set up the same dataset but for the GT SC
    TrueDataset= CamLocDataset(rgb_files="/home/alan/Desktop/Master-Thesis/chess/chess3_rgb/*.png",
                          pose_files="/home/alan/Desktop/Master-Thesis/chess/chess3_poses/*.txt",
                          depth_files="/home/alan/Desktop/Master-Thesis/chess/chess3_depth/*.png",
                          use_depth=True,
    ...
    file1 = open('output_test2.txt', 'a')
    gradient_file = open('gradient.txt','a')
    # Testing loop.
    ...
    for image_B1HW, _, _, _, intrinsics_B33, _, _, filenames, indices in testset_loader:
        ...
        # GT SC
        gt_sc = TrueDataset._get_single_item(indices,480)
        # L1 Loss
        L1_fn = torch.nn.L1Loss()
        L1_loss = L1_fn(scene_coordinates_B3HW,gt_sc)
        L1_loss.backward()
        gradient_file.write(str(image_B1HW.grad))


        # Test to save the coordinates #####################################3
        file1.write(str(scene_coordinates_B3HW) + '\n' + str(scene_coordinates_B3HW.size()))
        #########################################################
